...
# Load the dataset
with open("../VQA-RAD/vqa_rad/final_vqa_rad_no_rules.json", "r") as f:
    data = json.load(f)
...
for num, item in enumerate(data):
    ...
    combined_question = item["question"]
    if "question_rephrase" in item and item["question_rephrase"] != "NULL":
        combined_question += " " + item["question_rephrase"]
    ...
    r1_sorted = sorted_triple(combined_question, r1, pubmed_bert_model, pubmed_bert_tokenizer)
    r2_sorted = sorted_triple(combined_question, r2, pubmed_bert_model, pubmed_bert_tokenizer)
    r3_sorted = sorted_triple(combined_question, r3, pubmed_bert_model, pubmed_bert_tokenizer)
    r4_sorted = sorted_triple(combined_question, r4, pubmed_bert_model, pubmed_bert_tokenizer)
    r5_sorted = sorted_triple(combined_question, r5, pubmed_bert_model, pubmed_bert_tokenizer)
    r6_sorted = sorted_triple(combined_question, r6, pubmed_bert_model, pubmed_bert_tokenizer)

    processed_sample = {
        "qid": item["qid"],
        "phrase_type": item["phrase_type"],
        "qid_linked_id": item["qid_linked_id"],
        "image_case_url": item["image_case_url"],
        "image_name": item["image_name"],
        "image_organ": item["image_organ"],
        "evaluation": item["evaluation"],
        "question": item["question"],
        "question_rephrase": item.get("question_rephrase", "NULL"),
        "question_relation": item["question_relation"],
...
with open("../VQA-RAD/vqa_rad/sorted_vqa_with_rules_on_Question.json", "w") as f:
