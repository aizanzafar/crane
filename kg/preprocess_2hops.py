...
def sorted_triple(question,context_kg,pubmed_bert_model,pubmed_bert_tokenizer):
	...
	print("after 1 hop: ",len(sorted_triple_list))

	sorted_triple_list= sorted_triple_list[:300]
	sorted_tail=[]
	for t in sorted_triple_list:
		sorted_tail.append(t[2])
	sorted_tail = list(set(sorted_tail))
	# print("sorted_tail: ",len(sorted_tail))

	final_kg_triplet = n_hops(sorted_tail[:20],context_kg,n=1)
	...
	for k,v in sorted_dict.items():
		sorted_triple_list.append(triple_n_sim_final_dic[k])
	print("after 2 hop: ",len(sorted_triple_list))
	return sorted_triple_list[:800]
