...
Your task is to analyze the provided medical image and transform the initial knowledge graph (KG) into a refined, well-structured, concise, and accurate knowledge graph Then, use this new KG to guide the captioning process, ensuring the caption accurately describes the key observations, anatomical regions, and any relevant medical findings depicted in the image. Use the following logical rules to assist in your refinement process:
...
4. Generate a new refine caption that accurately describes the key observations, anatomical regions, and any relevant medical findings depicted in the image.

Dont generate anything else except refined KG in the given format.
...
# Save results to CSV
output_csv = "VLU_roco_inference_using_LLAMA_results.csv"
